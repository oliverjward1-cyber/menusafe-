...
export function DishCard({ name, description, sellPrice, dishAllergens, ingredients }: Props) {
    ...
    <>
      ...
      >
        <div className="flex items-start justify-between gap-3">
          ...
          {sellPrice != null && (
            <p className="font-semibold text-gray-900 shrink-0">{formatCurrency(sellPrice)}</p>
          )}
        </div>
      ...
      {open && (
        ...
        >
          ...
          {/* Sheet */}
          <div
            className="relative bg-white w-full sm:max-w-md sm:rounded-2xl rounded-t-2xl shadow-xl max-h-[85vh] flex flex-col"
                          ...
                          <div className="flex flex-wrap gap-1 justify-end">
                            {ing.allergens.map((key) => {
                              ...
                              return (
                                <span
                                  key={key}
                                  className="inline-flex items-center px-1.5 py-0.5 rounded-full bg-amber-100 text-amber-800 text-xs font-medium border border-amber-200"
                                >
                                  {a.shortLabel}
                                </span>
                              )
                  ...
                  <div className="flex flex-wrap gap-2">
                    {dishAllergens.map((key) => {
                      ...
                      return (
                        <span
                          key={key}
                          className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-amber-100 text-amber-800 text-sm font-medium border border-amber-200"
                          title={a.description}
                        >
                          {a.label}
                        </span>
