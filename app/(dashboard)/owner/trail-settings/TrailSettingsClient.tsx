    ...
    <div className="fixed inset-0 bg-black/40 z-50 flex items-end sm:items-center justify-center p-4">
      <div className="bg-white rounded-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto shadow-2xl">
        <div className="sticky top-0 bg-white border-b border-black/[0.06] px-5 py-4 flex items-center justify-between rounded-t-2xl">
          <h2 className="font-bold text-mise-ink">{initial?.id ? 'Edit task' : 'New task'}</h2>
          <button onClick={onCancel} className="text-mise-ink/30 hover:text-mise-ink transition-colors">
            <X className="h-5 w-5" />
          </button>
        </div>
        ...
        <div className="p-5 space-y-5">
          ...
          <div>
            ...
            <input
              ...
              className="w-full border border-black/[0.08] rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-mise-mid/30"
            />
          </div>

          {/* Description */}
          <div>
            <label className="block text-xs font-semibold text-mise-ink/50 uppercase tracking-widest mb-1.5">Description (optional)</label>
            <input
              value={description}
              ...
              {TASK_TYPES.map(t => (
                <button
                  ...
                  className={`flex items-center gap-2.5 p-3 rounded-xl border text-left transition-colors
                    ${taskType === t.value ? 'border-mise-mid bg-mise-mid/5' : 'border-black/[0.08] hover:bg-gray-50'}`}
                >
                  <div className={`w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0 ${TYPE_COLOR[t.value]}`}>
                    <t.icon className="h-3.5 w-3.5" />
                  </div>
                  <div>
                    <p className="text-xs font-semibold text-mise-ink">{t.label}</p>
                    <p className="text-[10px] text-mise-ink/40">{t.desc}</p>
                  </div>
                </button>
              ))}
            </div>
          ...
          {/* Schedule */}
          <div>
            <label className="block text-xs font-semibold text-mise-ink/50 uppercase tracking-widest mb-1.5">Schedule</label>
            <div className="flex gap-2 mb-3">
              {SCHEDULE_TYPES.map(s => (
                <button
                  ...
                  className={`flex-1 py-2.5 rounded-xl text-sm font-medium border transition-colors
                    ${scheduleType === s.value ? 'border-mise-mid bg-mise-mid text-white' : 'border-black/[0.08] text-mise-ink/60 hover:bg-gray-50'}`}
                >
                  {s.label}
                </button>
              ))}
                ...
                {DAYS.map((d, i) => (
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-lg text-xs font-semibold border transition-colors
                      ${scheduleDays.includes(i) ? 'bg-mise-mid text-white border-mise-mid' : 'border-black/[0.08] text-mise-ink/60 hover:bg-gray-50'}`}
                  >
                    {d}
                  </button>
                ))}
          ...
          {/* Time */}
          <div>
            <label className="block text-xs font-semibold text-mise-ink/50 uppercase tracking-widest mb-1.5">Scheduled time (optional)</label>
            <input
              type="time"
              value={scheduledTime}
              onChange={e => setScheduledTime(e.target.value)}
              className="border border-black/[0.08] rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-mise-mid/30"
            />
          </div>
          ...
          {taskType === 'checklist' && (
            <div>
              <label className="block text-xs font-semibold text-mise-ink/50 uppercase tracking-widest mb-1.5">Checklist items</label>
                ...
                {items.map(item => (
                  <div key={item.id} className="flex items-center gap-2">
                    <input
                      ...
                      className="flex-1 border border-black/[0.08] rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-mise-mid/30"
                    />
                    <button
                      ...
                      className={`flex-shrink-0 text-xs px-2 py-1.5 rounded-lg border font-medium transition-colors
                        ${item.required ? 'border-mise-mid text-mise-mid bg-mise-mid/5' : 'border-black/[0.08] text-mise-ink/30'}`}
                    >
                      {item.required ? 'Req' : 'Opt'}
                    </button>
                    <button onClick={() => removeItem(item.id)}
                      className="flex-shrink-0 text-mise-ink/20 hover:text-red-500 transition-colors">
                      <X className="h-4 w-4" />
                    </button>
                  </div>
                ))}
        ...
        <div className="sticky bottom-0 bg-white border-t border-black/[0.06] px-5 py-4 flex gap-2 rounded-b-2xl">
          <button onClick={onCancel}
            className="px-4 py-2.5 border border-black/[0.08] rounded-xl text-sm text-mise-ink/50 hover:text-mise-ink transition-colors">
            Cancel
          </button>
          <button
            onClick={handleSave}
            disabled={saving || !isValid}
            className="flex-1 bg-mise-deep text-white rounded-xl py-2.5 text-sm font-semibold disabled:opacity-40 flex items-center justify-center gap-2"
          >
...
function TemplateRow({
  ...
  return (
    <div className={`flex items-center gap-3 bg-white border rounded-2xl p-4 shadow-sm transition-opacity
      ...
      <div className="text-mise-ink/20 cursor-grab flex-shrink-0">
      ...
      <div className="flex-1 min-w-0">
        <p className="font-semibold text-mise-ink text-sm">{template.title}</p>
        <div className="flex items-center gap-2 mt-0.5 flex-wrap">
          {template.scheduled_time && (
            <span className="text-xs text-mise-ink/40">{fmt(template.scheduled_time)}</span>
          )}
          <span className="text-xs text-mise-ink/30 capitalize">
            {template.schedule_type === 'daily' ? 'Every day' : 'Weekly'}
          </span>
          {template.checklist_items && (
            <span className="text-xs text-mise-ink/30">{template.checklist_items.length} items</span>
          )}
      ...
      <div className="flex items-center gap-1 flex-shrink-0">
        <button
          ...
          className="p-2 rounded-lg text-mise-ink/30 hover:text-mise-ink hover:bg-gray-100 transition-colors"
        >
          {template.is_active ? <Eye className="h-4 w-4" /> : <EyeOff className="h-4 w-4" />}
        </button>
        <button
          onClick={() => onEdit(template)}
          className="p-2 rounded-lg text-mise-ink/30 hover:text-mise-mid hover:bg-mise-mid/5 transition-colors text-xs font-medium"
        >
          Edit
        </button>
        <button
          onClick={() => onDelete(template.id)}
          className="p-2 rounded-lg text-mise-ink/20 hover:text-red-500 hover:bg-red-50 transition-colors"
        >
      ...
      <div className="max-w-2xl mx-auto px-4 py-8 space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-mise-ink/50 text-sm mt-0.5">Manage your daily task templates</p>
          </div>
          <button
            onClick={() => setCreating(true)}
            className="flex items-center gap-2 bg-mise-mid text-white px-4 py-2.5 rounded-xl text-sm font-semibold hover:bg-mise-deep transition-colors"
          >
