...
export default function StaffCleaning() {
  ...
  if (loading) return (
    <div className="min-h-screen bg-gray-50 flex items-center justify-center">
      <p className="text-hospopilot-ink/40 text-sm animate-pulse">Loading tasks…</p>
    </div>
  )

  if (tasks.length === 0) return (
    <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center px-4 text-center">
      <Sparkles className="h-12 w-12 text-hospopilot-mid/30 mb-3" />
      <p className="text-hospopilot-ink/60 font-medium">No cleaning tasks set up yet</p>
      <p className="text-hospopilot-ink/30 text-sm mt-1">Ask your manager to add tasks in the owner portal</p>
      <button onClick={() => router.back()} className="mt-6 text-hospopilot-mid text-sm font-medium">← Back</button>
    </div>
  )
  ...
  if (remaining.length === 0 && !loading) return (
    <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center px-4 text-center">
      ...
      <h2 className="text-xl font-semibold text-hospopilot-ink">All done!</h2>
      <p className="text-hospopilot-ink/50 text-sm mt-1">All cleaning tasks signed off</p>
          ...
          <div key={task.id} className={`bg-white rounded-2xl border shadow-sm p-4 flex items-center gap-4 transition-opacity ${task.done ? 'opacity-40' : ''}`}>
            <button onClick={() => !task.done && signOff(task)} disabled={task.done || signingOff === task.id}
              ...
              {task.done
                ...
                : signingOff === task.id
                ? <div className="h-7 w-7 rounded-full border-2 border-hospopilot-mid border-t-transparent animate-spin" />
                : <Circle className="h-7 w-7 text-hospopilot-ink/20" />}
            </button>
            <div className="flex-1">
              <p className={`font-medium text-hospopilot-ink ${task.done ? 'line-through' : ''}`}>{task.name}</p>
              {task.area && <p className="text-xs text-hospopilot-ink/40 mt-0.5">{task.area}</p>}
              <p className="text-xs text-hospopilot-ink/30 capitalize mt-0.5">{task.frequency}</p>
            </div>
            {!task.done && (
              <button onClick={() => signOff(task)} disabled={signingOff === task.id}
                className="flex-shrink-0 bg-hospopilot-deep text-white rounded-xl px-4 py-2 text-sm font-semibold disabled:opacity-40">
                Done
              </button>
            )}
          </div>
